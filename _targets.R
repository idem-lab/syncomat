...
# this will setup a workspace when our code errors
...
tar_plan(
  ...
  tar_target(
    all_countries,
    read_csv(all_countries_path) %>% select(country = name)
  ),

  # Create country_list
  tar_target(country_list,
             create_country_list_from_wpp(
               wpp_data = wpp_data,
    countries = all_countries)),

  # subset the country list
  tar_target(
    proposed_countries,
  ...
  tar_target(data_pop, create_pop_data(proposed_countries)),

  # Create contact matrices
  tar_target(data_contact,
             create_contact_matrices(
               data_pop = data_pop,
               country_list = proposed_countries,
               start_age = 0,
               end_age = 80
               )),
