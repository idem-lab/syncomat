...
save_conmat_as_csv <- function(matrix_list, path = "./", subfolder = FALSE) {
  ...
  n_countries <- length(names(matrix_list))
  n_matrices <- sum(lengths(matrix_list))
  total_file_paths <- n_countries * n_matrices
  new_file_path <- vector("character", length = total_file_paths)

  ## TODO
  ## Think about moving some of these sections of the for loop
  ## outside of the for loop - there might be vectorised
  ## ways to do some of these steps
  for (country_name in names(matrix_list)) {
    ...
    for (matrix_name in names(country_matrices)) {
      ...
      # new_file_path[matrix_name] <- file_path
