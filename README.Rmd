...
[^readme-4]: This pipeline took 13 minutes to run on a computer equipped with an Intel:registered: Core:tm: i7-8565U and 16 GB of RAM running a 64-bit version of Windows.
...
1.  Create the age-specific population data you would like to generate synthetic contact matrices for. In this instance, the population data was obtained using the `wpp_age()` function from the `socialmixr` package. The data is saved in the target object `in_data_wpp`.
...
7.  `population_data` is a converted `conmat` population data using the [`as_conmat_population()`](https://idem-lab.github.io/conmat/dev/reference/as_conmat_population.html) function from `conmat`.
...
8.  Generate synthetic contact matrices from our population data using the [`extrapolate_polymod()`](https://idem-lab.github.io/conmat/dev/reference/extrapolate_polymod.html) function from `conmat`.
...
9.  Save the generated synthetic contact matrices as csv files.
    ...
    The `path` argument allows you to specify where you would like to save these csv files; change this if you would like to change the folder name. The default folder name is `output-contact-matrices` within the project directory.
